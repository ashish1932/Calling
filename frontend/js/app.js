...
class AppController {
  ...
  async init() {
    ...
    // Bind all event listeners BEFORE the login check so they are ready
  ...
  bindNavigation() {
    ...
    const handleNavigation = (item) => {
      ...
      this.switchScreen(screenId, targetTab);
    };
  ...
  async refreshData() {
    try {
      const res = await fetch(`${window.CounselFlow.API_BASE}/staff`);
      if (res.ok) {
        window.CounselFlow.DEMO_CREDENTIALS = await res.json();
      }
    } catch (e) {
      console.warn("Failed to fetch staff directory in refreshData");
    }
  ...
  async switchScreen(screenId, targetTab = null) {
    const activeRole = this.activeRole || window.CounselFlow.getActiveRole() || 'counsellor';
    ...
    if (screenId === 'opd') {
      if (typeof window.initOpdDashboard === 'function') {
        window.initOpdDashboard();
      }
      if (window.CounselFlow && typeof window.CounselFlow.refreshOpdSidebar === 'function') {
        window.CounselFlow.refreshOpdSidebar();
      }
      // Switch to the specific OPD tab panel (or dashboard overview)
      if (typeof window.switchOpdTab === 'function') {
        window.switchOpdTab(targetTab || 'dashboard');
      }
    }
    // Update sidebar active highlights
    const activeOpdTab = targetTab || 'dashboard';
    document.querySelectorAll('.nav-item').forEach(el => {
      const elScreen = el.getAttribute('data-screen');
      const elTab    = el.getAttribute('data-tab');
      if (elScreen === screenId) {
        if (screenId === 'opd') {
          // For OPD, only highlight the exact tab that is active
          if (elTab === activeOpdTab) {
    ...
    switch (screenId) {
      ...
      case 'profiles':
        ...
        break;
      case 'opd':
        this.dom.pageTitleText.innerText = "OPD Dispensation Portal";
        this.dom.pageSubtitleText.innerText = "Manage patient queue, dispensations, and medicine inventory";
        break;
    }
  }
    ...
    const columns = [
      {
        ...
        renderChecklist: (pt) => {
          const wChecked = pt.checkpoints?.withdrawalStabilised === true ? 'checked' : '';
          const lChecked = pt.checkpoints?.layer1And2Ready === true ? 'checked' : '';
          const canEdit = ['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole);
          ...
          return `
            ...
            ${(canEdit && pt.checkpoints?.withdrawalStabilised === true && pt.checkpoints?.layer1And2Ready === true) ? `
      ...
      },
      {
        ...
        renderChecklist: (pt) => {
          const fChecked = pt.checkpoints?.familyPsychoedAttended === true ? 'checked' : '';
          const canEdit = ['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole);
          const disabledAttr = canEdit ? '' : 'disabled';
          ...
          } else if (canEdit && pt.checkpoints?.familyPsychoedAttended !== true) {
          ...
          return `
            ...
            ${(canEdit && (pt.checkpoints?.familyPsychoedAttended === true || anchorStatus === 'unavailable')) ? `
      ...
      },
      {
        ...
        renderChecklist: (pt) => {
          const bChecked = pt.checkpoints?.day30ReviewPassed === true ? 'checked' : '';
          const canEdit = ['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'].includes(activeRole);
          const canFail = ['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole);
          const disabledAttr = canEdit ? '' : 'disabled';
          ...
          return `
            ...
            ${(canEdit && pt.checkpoints?.day30ReviewPassed === true) ? `
            ...
            ` : ''}
            ${(canFail && pt.checkpoints?.day30ReviewPassed !== true) ? `
  ...
  bindWorkflowEvents() {
    ...
    board.querySelectorAll('.chk-workflow-checkpoint').forEach(cb => {
      cb.addEventListener('change', async (e) => {
        ...
        const allowedRoles = isDetoxOrFamilyField ? ['admin', 'spo', 'supervisor', 'ddrc'] : ['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'];
        ...
        if (pt) {
          ...
          try {
            const serverPatient = await window.CounselFlow.patchPatient(id, {
              checkpoints: pt.checkpoints,
              clinicalStage: pt.clinicalStage
            });
            if (serverPatient) {
              const idx = this.patients.findIndex(p => p.id === id);
              if (idx !== -1) this.patients[idx] = serverPatient;
            }
    ...
    board.querySelectorAll('.btn-workflow-no-family').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        const id = e.target.getAttribute('data-id');
        if (!['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole)) {
    ...
    board.querySelectorAll('.btn-workflow-promote').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        const allowedRoles = isDetoxOrFamilyTarget ? ['admin', 'spo', 'supervisor', 'ddrc'] : ['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'];
        if (!allowedRoles.includes(activeRole)) {
        ...
        const pt = this.patients.find(p => p.id === id);
        if (pt) {
            document.getElementById('stage-promote-patient-id').value = id;
            document.getElementById('stage-promote-target-stage').value = targetStage;
            document.getElementById('stage-promote-opinion').value = '';
            
            const titles = {
                3: "Request MO Clearance",
                4: "Schedule Bridge Review",
                5: "Promote to Maintenance",
                6: "Complete Recovery"
            };
            document.getElementById('stage-modal-title').textContent = titles[targetStage] || "Stage Transition Clearance";
            document.getElementById('modal-stage-promote').classList.add('active');
            
            setTimeout(() => {
                document.getElementById('stage-promote-opinion').focus();
            }, 100);
        }
      });
    });
    board.querySelectorAll('.btn-workflow-re-enroll').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        const id = e.target.getAttribute('data-id');
        const allowedRoles = ['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'];
        if (!allowedRoles.includes(activeRole)) {
        ...
        const pt = this.patients.find(p => p.id === id);
        if (pt) {
          const newCheckpoints = { withdrawalStabilised: false, layer1And2Ready: false, familyPsychoedAttended: false, day30ReviewPassed: false };
          pt.checkpoints = newCheckpoints;
          pt.status = 'Monitored';
          if (window.CounselFlow && window.CounselFlow.evaluatePatientWorkflow) {
            window.CounselFlow.evaluatePatientWorkflow(pt);
          }
          ...
          } catch (err) { console.error("Re-enroll save failed:", err); }
          this.renderClinicalWorkflow();
        }
      });
    });
    ...
    board.querySelectorAll('.btn-workflow-fail-review').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (pt) {
          ...
          } catch (err) { console.error("Fail-review save failed:", err); }
          this.renderClinicalWorkflow();
          this.renderEscalationPanel();
        }
      });
        ...
        if (pt) {
          try {
            const serverPatient = await window.CounselFlow.patchPatient(id, {
              ...
              stage6SignoffCounsellorAt: new Date().toLocaleString(),
              stage6SignoffCounsellorBy: this.loggedInName || activeRole
            });
            if (serverPatient) {
              const idx = this.patients.findIndex(p => p.id === id);
              if (idx !== -1) this.patients[idx] = serverPatient;
          ...
          } catch (err) { console.error("Counsellor sign-off save failed:", err); }
          this.renderClinicalWorkflow();
        }
      });
    });
      ...
      btn.addEventListener('click', async (e) => {
        ...
        if (pt) {
          const updates = {
            ...
            stage6SignoffSupervisorAt: new Date().toLocaleString(),
            stage6SignoffSupervisorBy: this.loggedInName || activeRole
          };
          // Both signed off — mark complete
          if (pt.stage6SignoffCounsellor) {
            updates.status = 'Completed';
            updates.clinicalStage = 6;
            window.CounselFlow.writeAuditEvent('COHORT_CLOSE_DUAL_SIGNOFF', pt.id, 'N/A', activeRole, `Stage 6 dual sign-off complete. Counsellor: ${pt.stage6SignoffCounsellorBy || 'N/A'}, Supervisor: ${this.loggedInName || activeRole}. Case formally closed.`);
          }
          try {
            const serverPatient = await window.CounselFlow.patchPatient(id, updates);
            if (serverPatient) {
              const idx = this.patients.findIndex(p => p.id === id);
              if (idx !== -1) this.patients[idx] = serverPatient;
            }
          ...
          } catch (err) { console.error("Supervisor sign-off save failed:", err); }
          this.renderClinicalWorkflow();
        }
      });
    });
  ...
  async renderOpdDefaulters(container, roleConfig) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const defaulters = Array.isArray(resData) ? resData : (resData.data || []);
  ...
  getFilteredPatients() {
    ...
    if (this.currentPatientSort && this.currentPatientSort.field) {
      filtered.sort((a, b) => {
        ...
        // Provide defaults for missing data to prevent sort breakage
  ...
  renderPatientsList() {
    ...
    `;
    const bulkActiveRole = this.activeRole || window.CounselFlow.getActiveRole() || 'counsellor';
    const bulkRoleConfig = window.CounselFlow.getRoleConfig(bulkActiveRole);
    const bulkToolbar = this.selectedPatients.size > 0 ? `
      <div style="position:sticky; top:0; z-index:10; background:var(--bg-card); border:1px solid var(--accent-blue); padding:10px 16px; border-radius:8px; margin-bottom:14px; display:flex; align-items:center; justify-content:space-between; box-shadow:var(--shadow-neon);">
        ...
        <div style="display:flex; gap:10px;">
          ${bulkRoleConfig.canExportPatients ? `<button class="btn-secondary" id="btn-bulk-export" style="font-size:11px; padding:6px 12px;">Export Selected</button>` : ''}
          ${bulkRoleConfig.canBulkDeletePatients ? `<button class="btn-secondary" id="btn-bulk-delete" style="font-size:11px; padding:6px 12px; border-color:var(--accent-red); color:var(--accent-red);">Delete Selected</button>` : ''}
        </div>
      </div>
    ` : '';
    ...
    if (bulkDelete) {
      bulkDelete.addEventListener('click', async () => {
        if(confirm(`Are you sure you want to delete ${this.selectedPatients.size} patients?`)) {
          if (window.CounselFlow && window.CounselFlow.deletePatient) {
            for (const id of this.selectedPatients) {
              await window.CounselFlow.deletePatient(id);
            }
          }
  ...
  adjustCounselorFieldVisibility() {
    const activeRole = this.activeRole || window.CounselFlow.getActiveRole() || 'counsellor';
    const counselorEl = document.getElementById('new-patient-counselor');
    if (counselorEl) {
      if (activeRole === 'ddrc' || activeRole === 'counsellor') {
        counselorEl.parentElement.style.display = 'none';
      } else {
        counselorEl.parentElement.style.display = 'block';
      }
    }
  }
  ...
  openPatientForm() {
    ...
    if (overlay) {
      ...
      const consentInput = document.getElementById('new-patient-consent');
      if (consentInput) {
        consentInput.checked = true;
      }
      delete document.getElementById('form-add-patient').dataset.editId;
      this.adjustCounselorFieldVisibility();
  ...
  openPatientDetail(patient) {
    ...
    card.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:flex-start;">
        ...
        ${roleConfig.canEditPatient ? `<button class="btn-secondary" id="btn-edit-patient" style="font-size:11px; padding:6px 10px; display:flex; align-items:center; gap:6px;">
      ...
      <div class="profile-info-list">
        ...
        <div class="info-item">
          ...
          <span>${escapeHtml(patient.assignedCounselor || 'Unassigned')}</span>
        </div>
        <div class="info-item">
          <span>NGO Partner</span>
          <span>${escapeHtml(patient.ngoPartner || 'N/A')}</span>
        </div>
        <div class="info-item" style="flex-direction:column; gap:8px; align-items:flex-start; border-top:1px solid var(--border-light); padding-top:16px; text-align:left;">
          ...
          <textarea id="patient-notes-textarea" aria-label="Patient clinical notes" style="width:100%; height:80px; background:var(--bg-input); border:1px solid var(--border-light); border-radius:8px; color:var(--text-primary); padding:8px; font-size:12px; font-family:var(--font-body); resize:none;" ${roleConfig.canEditPatient ? '' : 'readonly'}>${escapeHtml(patient.notes || '')}</textarea>
          ${roleConfig.canEditPatient ? `<button class="btn-secondary" id="btn-save-general-notes" style="font-size:11px; padding:6px 12px; align-self:flex-end;">Save General Notes</button>` : ''}
        </div>
      </div>
      <div class="clinical-tracker" style="margin-top:20px; padding: 12px; background:var(--bg-input); border:1px solid var(--border-light); border-radius:10px;">
        ...
        <div style="display:flex; flex-direction:column; gap:6px; font-size: 11px;">
          <label style="display:flex; align-items:center; gap:8px; opacity: ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) ? '1' : '0.5'};">
            <input type="checkbox" id="chk-withdrawal" ${patient.checkpoints?.withdrawalStabilised === true ? 'checked' : ''} ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) ? '' : 'disabled'}>
            [Stage 2] Withdrawal Stabilised? (MO clearance)
          </label>
          <label style="display:flex; align-items:center; gap:8px; opacity: ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) ? '1' : '0.5'};">
            <input type="checkbox" id="chk-layer12" ${patient.checkpoints?.layer1And2Ready === true ? 'checked' : ''} ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) ? '' : 'disabled'}>
            [Stage 2] Layer 1 + 2 Ready? (Independent)
          </label>
          <label style="display:flex; align-items:center; gap:8px; opacity: ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) ? '1' : '0.5'};">
            <input type="checkbox" id="chk-family" ${patient.checkpoints?.familyPsychoedAttended === true ? 'checked' : ''} ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) ? '' : 'disabled'}>
            [Stage 3] Family attended Psychoed? (90-min)
          </label>
          <label style="display:flex; align-items:center; gap:8px; opacity: ${['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'].includes(activeRole) ? '1' : '0.5'};">
            <input type="checkbox" id="chk-day30" ${patient.checkpoints?.day30ReviewPassed === true ? 'checked' : ''} ${['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'].includes(activeRole) ? '' : 'disabled'}>
            [Stage 4] 30-Day Bridge Review Passed?
          </label>
        </div>
        ${['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'].includes(activeRole) ? `
        ...
        ` : ''}
        ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) && (patient.clinicalStage || 1) === 1 ? `
        ...
        ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) && (patient.clinicalStage || 1) === 2 ? `
        ...
        ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) && (patient.clinicalStage || 1) === 3 ? `
        ...
        ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) && (patient.clinicalStage || 1) === 5 ? `
        ...
        ${['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole) && (patient.clinicalStage || 1) <= 3 && patient.status !== 'LAMA' ? `
    ...
    if (saveCheckpointsBtn) {
      saveCheckpointsBtn.addEventListener('click', async () => {
        ...
        const canEditDetox = ['admin', 'spo', 'supervisor', 'ddrc'].includes(activeRole);
        const canEditBridge = ['admin', 'spo', 'supervisor', 'ddrc', 'counsellor'].includes(activeRole);
        ...
        // Calculate the new clinical stage locally before saving
        this.selectedPatient.checkpoints = newCheckpoints;
        if (window.CounselFlow && window.CounselFlow.evaluatePatientWorkflow) {
          window.CounselFlow.evaluatePatientWorkflow(this.selectedPatient);
        }
        try {
          // Atomic PATCH: update only this patient in the database
          const serverPatient = await window.CounselFlow.patchPatient(this.selectedPatient.id, {
            checkpoints: newCheckpoints,
            clinicalStage: this.selectedPatient.clinicalStage
          });
          // Sync local state with server-confirmed data
          if (serverPatient) {
            const idx = this.patients.findIndex(p => p.id === this.selectedPatient.id);
            if (idx !== -1) {
              this.patients[idx] = serverPatient;
              this.selectedPatient = serverPatient;
            }
          }
          this.showToast("Checkpoints Updated", `Clinical progress saved for ${this.selectedPatient.name}.`, "success");
        } catch (err) {
          // patchPatient already shows a toast on error
    ...
    if (lamaBtn) {
      lamaBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (serverPatient) {
            const idx = this.patients.findIndex(p => p.id === this.selectedPatient.id);
            if (idx !== -1) { this.patients[idx] = serverPatient; this.selectedPatient = serverPatient; }
          }
          this.showToast('LAMA Status Set', `${this.selectedPatient.name} has been flagged as LAMA. Dashboard updated.`, 'warning');
        } catch (err) { console.error("LAMA save failed:", err); }
        this.renderDashboard();
        this.openPatientDetail(this.selectedPatient);
      });
    }
    ...
    if (actionMoClearBtn) {
      actionMoClearBtn.addEventListener('click', async () => {
        ...
        const newCheckpoints = { ...(this.selectedPatient.checkpoints || {}), withdrawalStabilised: true, layer1And2Ready: true };
        this.selectedPatient.checkpoints = newCheckpoints;
        if (window.CounselFlow && window.CounselFlow.evaluatePatientWorkflow) {
          window.CounselFlow.evaluatePatientWorkflow(this.selectedPatient);
        }
        ...
        } catch (err) { console.error("MO clearance save failed:", err); }
        this.openPatientDetail(this.selectedPatient);
      });
    }
    ...
    if (actionFamilyBtn) {
      actionFamilyBtn.addEventListener('click', async () => {
        ...
        const newCheckpoints = { ...(this.selectedPatient.checkpoints || {}), familyPsychoedAttended: true };
        this.selectedPatient.checkpoints = newCheckpoints;
        if (window.CounselFlow && window.CounselFlow.evaluatePatientWorkflow) {
          window.CounselFlow.evaluatePatientWorkflow(this.selectedPatient);
        }
        ...
        } catch (err) { console.error("Family psychoed save failed:", err); }
        this.openPatientDetail(this.selectedPatient);
      });
    }
    ...
    if (actionDay30Btn) {
      actionDay30Btn.addEventListener('click', async () => {
        ...
        const newCheckpoints = { ...(this.selectedPatient.checkpoints || {}), day30ReviewPassed: true };
        this.selectedPatient.checkpoints = newCheckpoints;
        if (window.CounselFlow && window.CounselFlow.evaluatePatientWorkflow) {
          window.CounselFlow.evaluatePatientWorkflow(this.selectedPatient);
        }
        ...
        } catch (err) { console.error("30-Day review save failed:", err); }
        this.openPatientDetail(this.selectedPatient);
      });
    }
    ...
    if (actionDay90Btn) {
      actionDay90Btn.addEventListener('click', async () => {
        ...
        } catch (err) { console.error("90-day transition save failed:", err); }
        this.openPatientDetail(this.selectedPatient);
      });
    }
    ...
    if (editBtn) {
      editBtn.addEventListener('click', () => {
        ...
        if (patient.dob) {
          document.getElementById('new-patient-dob').value = patient.dob;
        } else if (patient.age) {
          const tempDob = new Date();
          tempDob.setFullYear(tempDob.getFullYear() - patient.age);
          document.getElementById('new-patient-dob').value = tempDob.toISOString().split('T')[0];
        } else {
        ...
        const consentInput = document.getElementById('new-patient-consent');
        if (consentInput) {
            consentInput.checked = patient.consentCaptured !== false && patient.consent !== false;
        }
        this.adjustCounselorFieldVisibility();
        overlay.classList.add('active');
      });
    ...
    // Bind add EMR record buttons — only visible to roles with edit permission
    const addEmrBtns = document.querySelectorAll('.btn-add-emr-record');
    addEmrBtns.forEach(btn => {
      if (!roleConfig.canEditPatient) {
  ...
  renderEmrTables(patient) {
    ...
    renderTable('tbody-emr-family-history', patient.familyHistory, (f) => `
      <tr style="border-bottom: 1px solid var(--border-light);">
        ...
        <td style="padding: 12px;">${f.notes || 'N/A'}</td>
      </tr>
    `);
    ...
    // Render Doctor's Assessment
    const docContainer = document.getElementById('emr-doctors-assessment-container');
    if (docContainer) {
      if (!patient.doctorsAssessment || patient.doctorsAssessment.length === 0) {
        docContainer.innerHTML = '<p style="text-align:center; padding:20px; color:var(--text-muted); margin:0;">No assessments recorded</p>';
      } else {
        docContainer.innerHTML = patient.doctorsAssessment.map(d => `
          <div style="background:var(--bg-input); border:1px solid var(--border-light); border-radius:10px; padding:16px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
              <span style="font-size:12px; color:var(--text-secondary);"><strong style="color:var(--text-primary);">${d.recordedBy}</strong> · ${d.date}</span>
            </div>
            <p style="font-size:13px; color:var(--text-primary); margin:0; line-height:1.5;">${escapeHtml(d.notes)}</p>
          </div>
        `).join('');
      }
    }
  ...
  openEmrRecordModal(type, patient) {
    ...
    const configs = {
      ...
      doctorsAssessment: {
        title: "Add Doctor's Assessment",
        subtitle: "Provide your final clinical opinion and diagnostic notes.",
        icon: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>',
    ...
    newForm.addEventListener('submit', async (e) => {
      ...
      } else if (currentType === 'doctorsAssessment') {
        entry.notes = document.getElementById('emr-doctors-notes').value || '';
      } else {
        return;
      }
  ...
  async savePatientNotes() {
    ...
    try {
      if (window.CounselFlow && window.CounselFlow.patchPatient) {
        const updated = await window.CounselFlow.patchPatient(this.selectedPatient.id, { notes: textVal });
        if (updated) {
          this.selectedPatient.notes = updated.notes;
          const idx = this.patients.findIndex(p => p.id === this.selectedPatient.id);
          if (idx !== -1) {
            this.patients[idx].notes = updated.notes;
          }
        }
      } else {
  ...
  renderPatientConditionSummary(patient) {
    ...
    container.innerHTML = `
      ...
      <!-- Historical Opinions -->
      ${patient.stageOpinions && patient.stageOpinions.length > 0 ? `
      <div style="margin-top:20px; border-top:1px solid var(--border-light); padding-top:16px;">
        <h4 style="font-size:12px; font-weight:700; color:var(--text-primary); margin-bottom:12px; display:flex; align-items:center; gap:8px;">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
          Stage Promotion History & Opinions
        </h4>
        <div style="display:flex; flex-direction:column; gap:10px;">
          ${patient.stageOpinions.map(op => `
            <div style="background:var(--bg-input); border:1px solid var(--border-light); border-radius:8px; padding:12px;">
              <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                <span style="font-size:11px; font-weight:600; color:var(--text-primary);">Stage ${op.fromStage} &rarr; Stage ${op.toStage}</span>
                <span style="font-size:10px; color:var(--text-secondary);">${new Date(op.date).toLocaleDateString()}</span>
              </div>
              <p style="font-size:11.5px; color:var(--text-secondary); margin:0 0 6px 0;">${escapeHtml(op.opinion)}</p>
              <div style="font-size:10px; color:var(--text-muted); text-align:right;">
                Approved by: <strong>${op.author || op.role}</strong> (${op.role.toUpperCase()})
              </div>
            </div>
          `).join('')}
        </div>
      ...
      if (activeRole === 'counsellor') {
        if (sess.recordingUrl) {
          if (diffMins <= settings.counselorAudioRetentionMins) {
            audioUI = `
              <div onclick="event.stopPropagation()" style="display:inline-flex; align-items:center;">
                <audio controls src="${sess.recordingUrl}" style="height:28px; max-width:180px;"></audio>
              </div>
            `;
          } else {
          ...
          audioUI = `<span style="font-size:10px; color:var(--text-muted); background:var(--bg-input); padding:4px 8px; border-radius:4px; border:1px solid var(--border-light);">No Audio Record</span>`;
        }
      } else {
        if (sess.recordingUrl) {
          if (diffDays <= settings.adminAudioRetentionDays) {
            audioUI = `
              <div onclick="event.stopPropagation()" style="display:inline-flex; align-items:center; gap:8px;">
                <audio controls src="${sess.recordingUrl}" style="height:28px; max-width:180px;"></audio>
                <a href="${sess.recordingUrl}" download="call_audio.webm" class="btn-secondary" style="text-decoration:none; display:inline-block; font-size:10px; padding:4px 8px; border-color:var(--accent-blue); color:var(--accent-blue);">⬇ Download</a>
              </div>
            `;
          } else {
  ...
  async renderPatientOpdLogs(patient) {
    ...
    try {
      const res = await fetch(`${window.CounselFlow.API_BASE}/opd/logs/${patient.id}`);
  ...
  bindModals() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const inputDob = document.getElementById('new-patient-dob').value;
      let inputAge = 0;
      if (inputDob) {
          const dobDate = new Date(inputDob);
          const today = new Date();
          inputAge = today.getFullYear() - dobDate.getFullYear();
      ...
      const consentInputEl = document.getElementById('new-patient-consent');
      const inputConsent = consentInputEl ? consentInputEl.checked : true;
      ...
      if (isNaN(inputAge) || inputAge < 12 || inputAge > 100) {
        this.showToast("Validation Error", "Date of Birth must result in a valid age between 12 and 100.", "error");
        return;
      }
      ...
      if (editingId) {
        ...
        if (pt) {
            ...
            pt.consent = inputConsent;
            pt.consentCaptured = inputConsent;
            if(!pt.checkpoints) {
                pt.checkpoints = { withdrawalStabilised: false, layer1And2Ready: false, familyPsychoedAttended: false, day30ReviewPassed: false };
            }
            if (!pt.vitals) pt.vitals = [];
            if (!pt.medicalHistory) pt.medicalHistory = [];
            if (!pt.familyHistory) pt.familyHistory = [];
            if (!pt.cowsAssessment) pt.cowsAssessment = [];
            if (!pt.doctorsAssessment) pt.doctorsAssessment = [];
            if (!pt.stageOpinions) pt.stageOpinions = [];
             ...
             if (activeCreds && (activeCreds.roleKey === 'counsellor' || activeCreds.roleKey === 'ddrc')) {
               pt.district = activeCreds.district || 'Amritsar';
               if (activeCreds.roleKey === 'counsellor') {
                 pt.counselorId = activeCreds.staffId;
                 pt.assignedCounselor = activeCreds.name;
               } else {
                 const districtCounselor = window.CounselFlow.DEMO_CREDENTIALS.find(c => 
                   c.roleKey === 'counsellor' && 
                   (c.district || '').toLowerCase() === (pt.district || '').toLowerCase()
                 );
                 if (districtCounselor) {
                   pt.counselorId = districtCounselor.staffId;
                   pt.assignedCounselor = districtCounselor.name;
                 }
               }
             } else {
      ...
      if (activeCreds && (activeCreds.roleKey === 'counsellor' || activeCreds.roleKey === 'ddrc')) {
        ...
        if (activeCreds.roleKey === 'counsellor') {
          ...
          const districtCounselor = window.CounselFlow.DEMO_CREDENTIALS.find(c => 
            c.roleKey === 'counsellor' && 
            (c.district || '').toLowerCase() === (pDistrict || '').toLowerCase()
          );
          if (districtCounselor) {
            pCounselorId = districtCounselor.staffId;
            pCounselorName = districtCounselor.name;
          }
        }
      } else {
        const addrLower = inputAddress.toLowerCase();
      ...
      const inputHashStr = `${inputName.trim().toLowerCase()}-${inputPhone.trim()}`;
      let hash = 0;
      for (let i = 0; i < inputHashStr.length; i++) {
        hash = ((hash << 5) - hash) + inputHashStr.charCodeAt(i);
        hash = hash & hash;
      }
      const hexHash = Math.abs(hash).toString(16).padStart(8, '0');
      
      const newPt = {
        id: `PT-${hexHash.slice(0, 4)}-${hexHash.slice(4)}`, 
        name: inputName,
        dob: inputDob,
        age: inputAge,
        gender: inputGender,
  ...
  bindStagePromoteModal() {
        const modal = document.getElementById('modal-stage-promote');
        const closeBtn = document.getElementById('btn-close-stage-modal');
        const cancelBtn = document.getElementById('btn-cancel-stage-modal');
        const form = document.getElementById('form-stage-promote');
        ...
        form.addEventListener('submit', async (e) => {
            e.preventDefault();
            const id = document.getElementById('stage-promote-patient-id').value;
            const targetStage = parseInt(document.getElementById('stage-promote-target-stage').value);
            const opinion = document.getElementById('stage-promote-opinion').value;
            ...
            const activeStaffId = window.CounselFlow.safeGetItem('counseling_logged_in_staff') || '';
            const activeCreds = window.CounselFlow.DEMO_CREDENTIALS.find(c => c.staffId === activeStaffId);
            const authorName = activeCreds ? activeCreds.name : activeRole;
            
            if (!pt.stageOpinions) pt.stageOpinions = [];
            pt.stageOpinions.unshift({
                date: new Date().toISOString(),
                fromStage: pt.clinicalStage,
                toStage: targetStage,
                opinion: opinion,
                role: activeRole,
                author: authorName
            ...
            if (!pt.checkpoints) pt.checkpoints = {};
            const updates = { stageOpinions: pt.stageOpinions };
            ...
            updates.checkpoints = pt.checkpoints;
            updates.clinicalStage = pt.clinicalStage;
            ...
            try {
              ...
              if (this.selectedPatient && this.selectedPatient.id === id) {
                  this.openPatientDetail(this.selectedPatient); // refresh detail view
              }
            } catch (err) { 
                console.error("Promote save failed:", err); 
                this.showToast('Error', 'Failed to approve stage transition.', 'error');
            } finally {
                submitBtn.disabled = false;
                submitBtn.innerHTML = 'Approve Transition';
            }
        });
    }
...
document.getElementById('btn-summary-export').addEventListener('click', () => {
  ...
  async commitCallSummaryToRecord() {
    ...
    const newSessionLog = {
      ...
      counselor: "Dr. Amanpreet Kaur",
      transcript: window.CounselFlow.callManager.getTranscript(),
      recordingUrl: isRecorded ? (window.CounselFlow.callManager.finalRecordingUrl || 'assets/audio/demo.mp3') : null,
      summary: summary
    };
    ...
    if (window.CounselFlow && window.CounselFlow.patchPatient) {
      try {
        await window.CounselFlow.patchPatient(this.selectedPatient.id, {
          history: this.selectedPatient.history,
          lastSessionDate: this.selectedPatient.lastSessionDate,
          cravingsIntensity: this.selectedPatient.cravingsIntensity,
          progress: this.selectedPatient.progress,
          status: this.selectedPatient.status
        });
        // Also add it to the global Call Logs registry
        if (window.CounselFlow.saveCallLogs) {
          const newCallLog = {
            logId: `LOG-${Math.floor(1000 + Math.random() * 999)}-${Date.now().toString(16).slice(-4)}`,
            timestamp: dateStr,
            patientId: this.selectedPatient.id,
            patientName: this.selectedPatient.name,
            counselorId: "C101",
            counselorName: "Dr. Amanpreet Kaur",
            sessionId: newSessionLog.sessionId,
            direction: "Outbound",
            duration: durationStr,
            disposition: "Completed",
            recordingUrl: window.CounselFlow.callManager.finalRecordingUrl || (newSessionLog.recordingUrl === 'assets/audio/demo.mp3' && !window.CounselFlow.callManager.isRecording ? null : newSessionLog.recordingUrl),
            summary: summary
          };
          await window.CounselFlow.saveCallLogs([newCallLog]);
        }

        this.showToast("Record Committed", `Session ${newSessionLog.sessionId} saved under ${this.selectedPatient.name}.`, "success");
        this.openPatientDetail(this.selectedPatient);
      } catch (err) {
        console.error("Failed to commit session to database:", err);
        this.showToast("Save Error", "Failed to commit session record to the database.", "error");
      }
    }
  }
  ...
  async renderHistoryRecords(query = '', language = 'all') {
    ...
    this.dom.historyRecordsList.innerHTML = filtered.map(item => {
      ...
      let audioUI = '';
      if (activeRole === 'counsellor') {
        if (item.session.recordingUrl) {
          if (diffMins <= settings.counselorAudioRetentionMins) {
            audioUI = `
              <div onclick="event.stopPropagation()" style="display:inline-flex; align-items:center;">
                <audio controls src="${item.session.recordingUrl}" style="height:28px; max-width:180px;"></audio>
              </div>
            `;
          } else {
            audioUI = `<span style="font-size:10px; color:var(--text-muted); background:var(--bg-input); padding:4px 8px; border-radius:4px; border:1px dashed var(--border-light);">🔒 Auto-Deleted (Privacy)</span>`;
        ...
        if (item.session.recordingUrl) {
          if (diffDays <= settings.adminAudioRetentionDays) {
            audioUI = `
              <div onclick="event.stopPropagation()" style="display:inline-flex; align-items:center; gap:8px;">
                <audio controls src="${item.session.recordingUrl}" style="height:28px; max-width:180px;"></audio>
                <a href="${item.session.recordingUrl}" download="call_audio.webm" class="btn-secondary" style="text-decoration:none; display:inline-block; font-size:10px; padding:4px 8px; border-color:var(--accent-blue); color:var(--accent-blue);">⬇ Download</a>
              </div>
            `;
          } else {
            audioUI = `<span style="font-size:10px; color:var(--accent-red); background:rgba(238,93,80,0.1); padding:4px 8px; border-radius:4px; border:1px solid rgba(238,93,80,0.2);">🗑 Permanently Deleted (Compliance)</span>`;
  ...
  async showLoginScreen() {
    try {
      ...
      if (res.ok) {
        ...
        // For the demo login cards, we manually inject the test passwords since the API correctly strips them for security
        window.CounselFlow.DEMO_CREDENTIALS = staffList.map(s => {
          let demoPwd = 'password';
          if (s.roleKey === 'spo') demoPwd = 'CBM@SPOwner24';
          else if (s.roleKey === 'supervisor') demoPwd = 'CBM@Supervisor24';
          else if (s.roleKey === 'counsellor') demoPwd = 'CBM@Counsellor24';
          else if (s.roleKey === 'ddrc') demoPwd = 'CBM@DDRC24';
          else if (s.roleKey === 'ditsu') demoPwd = 'CBM@DITSU24';
          else if (s.roleKey === 'opd_staff') demoPwd = 'CBM@OPD24';
          return { ...s, password: demoPwd };
        });
      }
    } catch (e) {
      console.warn("Failed to fetch staff directory for login screen:", e);
    }
      ...
      <div style="display:flex; flex-direction:row; gap:40px; width:100%; max-width:1120px; box-sizing:border-box; flex-wrap:wrap; justify-content:center; align-items:stretch; margin:auto; padding: 20px 0;">
        ...
        <div style="flex: 1; min-width: 320px; max-width: 440px; display:flex; flex-direction:column; justify-content:center;">
          ...
          <div class="government-banner" style="display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 14px 12px; margin-bottom: 20px; background: rgba(255, 255, 255, 0.02); border-radius: 12px; border: 1px solid var(--border-light); text-align: center;">
            <img src="assets/punjab_cm.png" alt="S. Bhagwant Singh Mann, Chief Minister of Punjab" style="width: 90px; height: 90px; border-radius: 8px; border: 1.5px solid var(--accent-orange); object-fit: cover; object-position: top; box-shadow: 0 4px 8px rgba(0,0,0,0.15);">
  ...
  applyRole(roleKey, isNew = false, userName = '') {
    ...
    // RBAC: Hide/show action buttons based on role permissions
    const addPatientBtn = document.getElementById('btn-add-patient-modal');
    if (addPatientBtn) {
      addPatientBtn.style.display = role.canAddPatient ? '' : 'none';
    }
    const exportPatientsBtn = document.getElementById('btn-export-patients');
    if (exportPatientsBtn) {
      exportPatientsBtn.style.display = role.canExportPatients ? '' : 'none';
    }
