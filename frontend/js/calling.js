...
class CallManager {
  ...
  async startCall(patient, languageCode, direction = "Outbound", isDemo = false) {
    ...
    // Reset/clear recording URL & UI
    this.finalRecordingUrl = null;
    ...
    if (this.isRecording) {
      ...
      if (statusLabel) statusLabel.innerText = `${direction} Call - Recording Active`;
      if (recordBtn) recordBtn.className = 'call-btn record recording';
    } else {
      if (statusDot) statusDot.className = 'status-dot';
      if (statusLabel) statusLabel.innerText = `${direction} Call - Recording Disabled (No Consent)`;
      if (recordBtn) recordBtn.className = 'call-btn record';
    }
  ...
  toggleRecording() {
    ...
    if (this.isRecording) {
      ...
      // Start or resume fullCallRecorder
      if (this.fullCallRecorder) {
        if (this.fullCallRecorder.state === 'paused') {
          try { this.fullCallRecorder.resume(); } catch(e) {}
        } else if (this.fullCallRecorder.state === 'inactive') {
          try { this.fullCallRecorder.start(1000); } catch(e) {}
        }
      } else {
        this.setupCallRecorder();
      }
      ...
      // Pause fullCallRecorder
      if (this.fullCallRecorder && this.fullCallRecorder.state === 'recording') {
        try { this.fullCallRecorder.pause(); } catch(e) {}
      }
    }
  }
  ...
  showIncomingCallPopup(patientId, patientName, roomName) {
    ...
    document.getElementById('btn-answer-call').onclick = () => {
      ...
      const patient = window.CounselFlow && window.CounselFlow.app && window.CounselFlow.app.patients 
        ? window.CounselFlow.app.patients.find(p => p.id === patientId)
        : null;
        
      const patientObj = patient || {
        id: patientId,
        name: patientName || 'Mobile Caller',
        counselorId: 'counselor',
        addictionCategory: 'Incoming Caller',
        consentCaptured: true
      };
      
      // Auto-switch to Dashboard to show the call session
      if (window.CounselFlow && window.CounselFlow.app) {
        window.CounselFlow.app.switchScreen('dashboard');
      }
      
      // Fully start the session instead of just initializing LiveKit
      this.startCall(patientObj, 'en-US', 'Inbound');
    };
  ...
  setupCallRecorder() {
    ...
    try {
      this.callRecordCtx = new (window.AudioContext || window.webkitAudioContext)();
      this.callRecordDest = this.callRecordCtx.createMediaStreamDestination();
      
      let options = { mimeType: 'audio/webm;codecs=opus' };
      if (!MediaRecorder.isTypeSupported(options.mimeType)) options = { mimeType: 'audio/webm' };
      
      this.fullCallRecorder = new MediaRecorder(this.callRecordDest.stream, options);
      this.fullCallChunks = [];
      this.fullCallRecorder.ondataavailable = e => {
        if (e.data && e.data.size > 0) this.fullCallChunks.push(e.data);
      };
      this.fullCallRecorder.onstop = async () => {
         const blob = new Blob(this.fullCallChunks, { type: 'audio/webm' });
         ...
         if (audioContainer) {
           ...
           audioContainer.innerHTML = `
             <div style="display:flex; align-items:center; gap:10px; padding:12px 16px; background:rgba(0,242,254,0.05); font-size:12px; color:var(--accent-blue); border-top: 1px solid var(--border-light);">
               <div class="spinner" style="width:14px; height:14px; border-width:2px; border-color:var(--accent-blue) transparent var(--accent-blue) transparent; border-style:solid; border-radius:50%; animation:spin 1s linear infinite;"></div>
               <span>Uploading call recording audio to secure server...</span>
             </div>
           `;
         }
