...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const searchFilterHtml = `
    <div style="display: flex; gap: 10px; margin-bottom: 15px; background: var(--bg-card); padding: 10px; border-radius: 8px; border: 1px solid var(--border-light); align-items: center; flex-wrap: wrap;">
      <input type="text" id="profiles-search-input" placeholder="Search by name, ID or phone..." style="flex:1; min-width:200px; background: var(--bg-input); border: 1px solid var(--border-light); color: var(--text-primary); padding: 8px 12px; border-radius: 6px; font-size: 13px;">
      <select id="profiles-district-select" style="background: var(--bg-input); border: 1px solid var(--border-light); color: var(--text-primary); padding: 8px 12px; border-radius: 6px; font-size: 13px;">
  ...
  const districtSelect = document.getElementById('profiles-district-select');
  if (districtSelect) {
  ...
  function canDeletePatient() {
    const role = getActiveRole();
    const config = window.CounselFlow && window.CounselFlow.getRoleConfig ? window.CounselFlow.getRoleConfig(role) : null;
    return config && config.canDeletePatient;
  }
  function canEditPatient() {
    const role = getActiveRole();
    const config = window.CounselFlow && window.CounselFlow.getRoleConfig ? window.CounselFlow.getRoleConfig(role) : null;
    return config && config.canEditPatient;
  }
  function canAddPatientProfile() {
    const role = getActiveRole();
    const config = window.CounselFlow && window.CounselFlow.getRoleConfig ? window.CounselFlow.getRoleConfig(role) : null;
    return config && config.canAddPatient;
  }
    ...
    modal.innerHTML = `
      <div class="modal-content" style="width:500px; max-height:80vh; overflow-y:auto; background:var(--bg-card); padding:24px; border-radius:12px; box-shadow:0 0 20px rgba(0,0,0,0.5);">
        ...
        <form id="counselor-profile-form" style="display:grid; grid-template-columns: 1fr 1fr; gap:12px; margin-top:15px;">
          ...
          <div style="grid-column: span 1;">
            <label style="font-size:11px; color:var(--text-secondary);">Avatar Color</label>
            <input type="color" id="c-avatarColor" name="avatarColor" class="select-filter" style="width:100%; height:36px; padding:2px;" value="#9c27b0">
          </div>
  ...
  document.addEventListener('click', (e) => {
    ...
    if (closeTarget) {
      ...
      const form = document.getElementById('counselor-profile-form');
      if (form) form.reset();
      return;
    }
  });
  ...
  async function getLocalCounselors() {
    ...
    try {
      if (navigator.onLine) {
        ...
        const res = await fetch(`${window.CounselFlow.API_BASE}/counselors`, { headers });
        if (res.ok) {
    ...
    // If database has counselors, use that directly.
    if (local && local.length > 0) {
      return local.map(c => ({
        ...c,
        roleKey: c.roleKey || (c.specialization === 'DDRC Admin' ? 'ddrc' : 'counsellor')
      }));
    }
    
    // Fallback seed list only if database is completely empty
  ...
  async function renderUI() {
     updateExportButton();
     if (currentTab === 'patients') await renderPatientsProfileUI();
     else await renderCounselorsProfileUI();
  }
  ...
  async function renderPatientsProfileUI() {
    ...
    if (districtFilter !== 'all') {
      patients = patients.filter(p => p.district === districtFilter);
    }
    ...
    const canAdd = canAddPatientProfile();
        ...
        html += `
          <div class="profile-card" style="border: 1px solid var(--border-light); background: var(--bg-card); border-radius: 8px; padding: 16px; display:flex; flex-direction:column;">
            ...
            <div style="margin-top:auto; padding-top:16px; display:flex; gap:8px; border-top:1px solid var(--border-light);">
              ${canEdit ? `<button class="btn-secondary" data-action="edit-patient" data-id="${escapeHtml(p.id)}" style="flex:1; font-size:11px;">Edit Profile</button>` : ''}
              ${canDelete ? `<button class="btn-secondary" data-action="delete-patient" data-id="${escapeHtml(p.id)}" style="flex:1; font-size:11px; color:var(--accent-red); border-color:rgba(238,93,80,0.3);">Delete</button>` : ''}
            </div>
          </div>
        `;
      });
  ...
  async function renderCounselorsProfileUI() {
    ...
    if (districtFilter !== 'all') {
      counselors = counselors.filter(c => c.district === districtFilter);
    }
        ...
        html += `
          <div class="profile-card" style="border: 1px solid var(--border-light); background: var(--bg-card); border-radius: 8px; padding: 16px; display:flex; flex-direction:column;">
            <div style="display:flex; gap:12px; align-items:center; margin-bottom:16px;">
               <div style="width:40px; height:40px; border-radius:50%; background:${c.avatarColor || 'var(--accent-purple)'}; color:white; display:flex; align-items:center; justify-content:center; font-weight:bold;">
            ...
            <div class="profile-info-list" style="margin-bottom:16px; display:flex; flex-direction:column; gap:8px;">
               ...
               <div class="info-item" style="display:flex; justify-content:space-between; font-size:12px;">
                 ...
                 <span>${window.CounselFlow.ROLES[activeRole]?.canViewPII ? escapeHtml(c.email || 'N/A') : '[PII Restricted]'}</span>
               </div>
               <div class="info-item" style="display:flex; justify-content:space-between; font-size:12px;">
                 <span style="color:var(--text-muted);">Phone</span>
                 <span>${window.CounselFlow.ROLES[activeRole]?.canViewPII ? escapeHtml(c.phone || 'N/A') : '[PII Restricted]'}</span>
               </div>
            </div>
  ...
  container.addEventListener('click', async (e) => {
    ...
    if (action === 'show-patient-form') {
      ...
      if (confirm('Are you sure you want to delete this patient profile? This action cannot be undone.')) {
         if (window.CounselFlow && window.CounselFlow.app) {
            if (window.CounselFlow.deletePatient) {
  ...
  document.getElementById('counselor-profile-form').addEventListener('submit', async (e) => {
    ...
    if (data.phone && !/^\+?[0-9\s\-()]{10,20}$/.test(data.phone.trim())) {
    ...
    if (!data.id) {
       data.id = data.staffId || ('STAFF-' + Math.floor(100 + Math.random() * 900));
    }
    data.staffId = data.id;
    if (!data.specialization || !data.specialization.trim()) {
      data.specialization = data.roleKey === 'ddrc' ? 'DDRC Admin' : 'Counselor';
    }
