...
class WebRTCService {
  ...
  async startCall(targetPatientId, callerInfo = { name: 'Dr. Amanpreet' }) {
    ...
    try {
      ...
      if (process.env.EXPO_PUBLIC_MOCK_WEBRTC === 'true') {
        ...
        console.error('[MobileWebRTC] WebRTC failed and mock mode is disabled:', err);
        return false;
      }
    }
  }
  ...
  async acceptCall() {
    if (!this.peerConnection) {
      if (process.env.EXPO_PUBLIC_MOCK_WEBRTC === 'true') {
        ...
        console.error('[MobileWebRTC] Cannot accept call: No peer connection available.');
        return false;
      }
    }
  ...
  cleanupCall() {
    ...
    this._iceCandidateBuffer = [];
    this._localIceCandidates = [];
    this._relayRecordLoop = null; // stop relay mic loop
    this.isRelayMode = false;
